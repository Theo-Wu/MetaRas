...
class Decoder(torch.nn.Module):
    def __init__(self, filename_obj, dim_in=512, centroid_scale=0.1, bias_scale=1.0, centroid_lr=0.1, bias_lr=1.0):
        ...
        # load .obj
        self.template_mesh = metadr.Mesh.from_obj(filename_obj)
...
class Model(torch.nn.Module):
    def __init__(self, filename_obj, args):
        ...
        # renderer
        self.transform = metadr.LookAt(viewing_angle=15)
        self.lighting = metadr.Lighting()
        self.renderer = metadr.MetaDR(
        ...
        # mesh regularizer
        self.laplacian_loss = metadr.LaplacianLoss(self.decoder.vertices_base, self.decoder.faces)
        self.flatten_loss = metadr.FlattenLoss(self.decoder.faces)
    ...
    def render_multiview(self, image_a, image_b, viewpoint_a, viewpoint_b):
        ...
        mesh = metadr.Mesh(vertices, faces)
    ...
    def evaluate_iou(self, images, voxels):
        ...
        faces_ = metadr.functional.face_vertices(vertices, faces).data
        faces_norm = faces_ * 1. * (32. - 1) / 32. + 0.5
        voxels_predict = metadr.functional.voxelization(faces_norm, 32, False).cpu().numpy()
...
class ShapeNet(object):
    ...
    def get_random_batch(self, batch_size):
        ...
        viewpoints_a = metadr.functional.get_points_from_angles(distances, elevations_a, -viewpoint_ids_a * 15)
        viewpoints_b = metadr.functional.get_points_from_angles(distances, elevations_b, -viewpoint_ids_b * 15)
...
def train():
    ...
    losses = AverageMeter()
    model.set_dist_scale(args.dist_scale)
    if args.init_weight:
        model.renderer.w1 = torch.nn.Parameter(args.weight*torch.ones([4*1]).float().to('cuda'))
        model.renderer.w2 = torch.nn.Parameter(args.weight*torch.ones([4*4]).float().to('cuda'))
        model.renderer.w3 = torch.nn.Parameter(args.weight*torch.ones([4*4]).float().to('cuda'))
        model.renderer.w4 = torch.nn.Parameter(args.weight*torch.ones([4*4]).float().to('cuda'))
        model.renderer.w5 = torch.nn.Parameter(args.weight*torch.ones([1*4]).float().to('cuda'))
    else:
        pretrained_weights = torch.load(f'data/recon.npy')
        model.renderer.w1 = pretrained_weights[0]
        model.renderer.w2 = pretrained_weights[1]
        model.renderer.w3 = pretrained_weights[2]
        model.renderer.w4 = pretrained_weights[3]
        model.renderer.w5 = pretrained_weights[4]
    
    history_loss = []
    model.renderer.w1.requires_grad = False
    model.renderer.w2.requires_grad = False
    model.renderer.w3.requires_grad = False
    model.renderer.w4.requires_grad = False
    model.renderer.w5.requires_grad = False
    for i in range(1, args.num_iterations + 1):
        ...
        if i > 10000:
            if i % 10000 == 0:
                model.renderer.w1.requires_grad = True
                model.renderer.w2.requires_grad = True
                model.renderer.w3.requires_grad = True
                model.renderer.w4.requires_grad = True
                model.renderer.w5.requires_grad = True
        ...
        if i % args.print_freq == 0:
            print('Iter: [{0}/{1}]\t'
                  ...
                  'sv {sv:.6f}\t'.format(i, args.num_iterations,
                                         ...
                                         lr=lr, sv=model.renderer.dist_scale.item()))
        ...
        if i % args.eval_freq == 0:
            ...
            model.train()
    torch.save([model.renderer.w1,
                model.renderer.w2,
                model.renderer.w3,
                model.renderer.w4,
                model.renderer.w5],'data/recon.npy')
...
# python metadr/experiments/train_reconstruction.py
...
if __name__ == '__main__':
    ...
    dist_func = args.dist_func + ('_squares' if args.squared else '')
    t_conorm = args.t_conorm + '_{:.1f}'.format(args.t_conorm_p)

    args.dist_scale = torch.nn.Parameter(torch.tensor([args.dist_scale]).to('cuda'))
    ...
    optimizer = torch.optim.Adam([{"params":model.model_param(), "lr":args.learning_rate},
                                    {"params": model.renderer.w1, "lr": args.learning_rate_meta},
                                    {"params": model.renderer.w2, "lr": args.learning_rate_meta},
                                    {"params": model.renderer.w3, "lr": args.learning_rate_meta},
                                    {"params": model.renderer.w4, "lr": args.learning_rate_meta},
                                    {"params": model.renderer.w5, "lr": args.learning_rate_meta}])
